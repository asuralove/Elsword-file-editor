	...
	namespace gui
	{
		...
		public:
			listview(HWND hwnd_parent = NULL, HINSTANCE instance = NULL)
				: widget(hwnd_parent, instance), column_index(0), menu_index(1)
			{

			}
			
			listview(widget& parent_widget, rectangle& rect = rectangle(), listview_function_t on_click_function = [](std::size_t) -> void {  })
				: widget(parent_widget.get_handle(), NULL), column_index(0), menu_index(1)
			{
				if (!this->assemble(parent_widget, rect, on_click_function))
				{
			...
			bool assemble(widget& parent_widget, rectangle& rect = rectangle(), listview_function_t on_click_function = [](std::size_t) -> void {  })
			{
				if (!this->create(WC_LISTVIEW, "", rect, WS_VISIBLE | WS_CHILD | WS_VSCROLL | LVS_REPORT | LVS_SHOWSELALWAYS | 
					LVS_SINGLESEL | LVS_NOSORTHEADER | LVS_OWNERDRAWFIXED, WS_EX_CLIENTEDGE, parent_widget.get_handle()))
				{
					return false;
				}
				
				ListView_SetExtendedListViewStyle(this->get_handle(), LVS_EX_FULLROWSELECT | LVS_EX_INFOTIP | LVS_EX_GRIDLINES);
				ListView_SetTextBkColor(this->get_handle(), CLR_NONE);

				this->set_background_color(RGB(255, 255, 255));
			...
			bool set_item_text(std::size_t item_index, std::size_t sub_item_index, std::string const& text)
			{
				LV_ITEM listview_item;
				memset(&listview_item, 0, sizeof(LV_ITEM));

				listview_item.iSubItem = sub_item_index;
				listview_item.pszText = const_cast<char*>(text.c_str());
				listview_item.cchTextMax = text.length();
				
				return (SendMessage(this->get_handle(), LVM_SETITEMTEXT, item_index, reinterpret_cast<LPARAM>(&listview_item)) != FALSE);
			}
		...
		protected:
			...
			void try_menu_functionality()
			{
				...
				if (item_index != -1)
				{
					...
					if (selected_menu != 0 && this->menu_functions[selected_menu] != 0)
					{
			...
			LRESULT draw_item(DRAWITEMSTRUCT* item_draw)
			{
				...
				for (std::size_t sub_item_index = 0; sub_item_index < this->column_index; sub_item_index++)
				{
					...
					HBRUSH background_brush = CreateSolidBrush(this->get_background_color());
					FillRect(item_draw->hDC, &rc_background, background_brush);
					...
					if ((item_draw->itemState & ODS_SELECTED) && (item_draw->itemAction & (ODA_SELECT | ODA_DRAWENTIRE)))
					{
						...
						HBRUSH selection_brush = CreateSolidBrush(GetSysColor(COLOR_HIGHLIGHT));
						FillRect(item_draw->hDC, &rc_background, selection_brush);
