...
namespace elswordkom
{
	namespace gui
	{
		...
		bool widget_base::set_font(std::string const& font)
		{
		...
		bool widget_base::set_size(size& new_size)
		{
			...
			return (SetWindowPos(this->hwnd, NULL, 0, 0, this->rect.get_width(), this->rect.get_height(), SWP_NOZORDER | SWP_NOMOVE) != FALSE &&
				InvalidateRect(this->hwnd, NULL, FALSE) != FALSE);
		}
		...
		std::string widget_base::get_text()
		{
			...
			std::string text(text_buffer, GetWindowText(this->hwnd, text_buffer, text_length + 1) + 1);
		...
		bool widget_base::set_text(std::wstring const& text)
		{
			return (SetWindowTextW(this->hwnd, text.c_str()) != FALSE);
		}
